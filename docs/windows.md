...
- Cookies: the Chrome cookies DB is copied to a temp location to avoid file locks, then read via `chrome-cookies-secure`. If cookies still fail to load, close all Chrome windows or pass inline cookies (`--browser-inline-cookies(-file)` / `ORACLE_BROWSER_COOKIES_JSON`). `--browser-no-cookie-sync` forces a manual login path.
