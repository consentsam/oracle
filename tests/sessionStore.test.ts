...
describe('sessionStore', () => {
  ...
  test('creates sessions and reads metadata/request', async () => {
    const meta = await store.createSession(
      { prompt: 'Inspect me', model: 'gpt-5-pro', search: false },
      process.cwd(),
    );
    ...
    const meta = await store.createSession(
      {
        prompt: 'Combine logs',
        model: 'gpt-5-pro',
        models: ['gpt-5-pro', 'gemini-3-pro'],
      },
      process.cwd(),
    );
    const writerPro = store.createLogWriter(meta.id, 'gpt-5-pro');
  ...
  test('deleteOlderThan prunes sessions past cutoff', async () => {
    const recent = await store.createSession({ prompt: 'recent', model: 'gpt-5-pro' }, process.cwd());
    const old = await store.createSession({ prompt: 'old', model: 'gpt-5-pro' }, process.cwd());
